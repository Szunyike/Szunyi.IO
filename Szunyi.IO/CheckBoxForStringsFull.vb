...
''' <summary>
''' This UserControl Return One or More SelectedItem
''' Input Can Be Strings, Sequences Or Files
''' </summary>
Public Class CheckBoxForStringsFull
    ...
    ''' <summary>
    ''' Store SelectedString in case the Input is based on String
    ''' </summary>
    ''' <returns></returns>
    Public Property SelectedStrings As New List(Of String)
    ''' <summary>
    ''' Store SelectedIndexes
    ''' </summary>
    ''' <returns></returns>
    Public Property SelectedIndexes As New List(Of Integer)
    ''' <summary>
    ''' Store Selected Files in case the Input is based on Files
    ''' </summary>
    ''' <returns></returns>
    Public Property SelectedFiles As New List(Of FileInfo)
    ''' <summary>
    ''' Store Selected obj in case the Input is based on general obj
    ''' </summary>
    ''' <returns></returns>
    ...
    ''' <summary>
    ''' Store Selected Sequences in case the Input is based on Sequences
    ''' </summary>
    ''' <returns></returns>
...
#Region "New"
    ...
    Public Sub New(Strings As List(Of String),
                   Max_Nof_SelectedItems As Integer,
                   Optional Title As String = "", Optional Selected_Item As String = "")
...
#End Region
    ...
    Private Sub Set_Selection(Selected_Items As List(Of String))
        ...
        SetIt()
    End Sub
    Private Sub Set_Selection(Selected_Item As String)
        If IsNothing(Selected_Item) = True OrElse Selected_Item = "" Then Exit Sub
        Dim alf As Int16 = 54
        For Each cItem In cItems
            Dim i1 = Selected_Item.IndexOf(cItem.Name, comparisonType:=StringComparison.InvariantCultureIgnoreCase)
            If i1 <> -1 Then
                cItem.Checked = True
            End If
        Next
    ...
    Private Sub OK(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If t.Count > Max_Nof_SelectedItems AndAlso Max_Nof_SelectedItems <> -1 Then
            ...
            Exit Sub
        End If
        If t1.Count > Max_Nof_SelectedItems AndAlso Max_Nof_SelectedItems <> -1 Then
            MsgBox("Too many Selected Items")
            Exit Sub
        End If
